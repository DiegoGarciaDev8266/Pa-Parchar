...
**Pa Parchar** es una aplicación web de gestión empresarial desarrollada como parte del reto académico de toma de decisiones. Permite administrar ventas, inventario y finanzas de un restaurante de comidas rápidas, integrando conceptos del **Doing Business** (Banco Mundial) y el **Happiness Index** (ONU) en su análisis financiero. Incluye un **agente administrativo con inteligencia artificial** que permite consultar métricas, registrar gastos y recibir alertas operativas en lenguaje natural.
...
| 🤖 **Agente IA** | Asistente administrativo con GPT-4o mini integrado en Finanzas — consulta métricas, registra gastos por chat y genera alertas automáticas de food cost |
...
| OpenAI API | GPT-4o mini | Agente administrativo con IA |
...
├── finanzas.html       # Módulo financiero + Agente IA
...
El módulo de Finanzas incluye un asistente administrativo inteligente accesible desde el botón flotante en la esquina inferior derecha. Opera con **GPT-4o mini de OpenAI** y tiene acceso en tiempo real a los datos de Firebase.

**Capacidades:**
- Consultar métricas financieras del mes: ventas, gastos, food cost, punto de equilibrio
- Registrar gastos desde el chat en lenguaje natural — se guardan directamente en Firestore
- Alertas automáticas cuando el food cost supera el 32%
- Análisis comparativo mes actual vs mes anterior

> **Nota de seguridad:** La API key de OpenAI no se incluye en el repositorio. Para usar el agente localmente, reemplaza `TU_API_KEY_AQUI` en `finanzas.html` con tu propia key de [platform.openai.com/api-keys](https://platform.openai.com/api-keys).
...
- API key de OpenAI (para el agente IA)

### Pasos

```bash
# 1. Clonar el repositorio
git clone https://github.com/DiegoGarciaDev8266/Pa-Parchar.git

# 2. Abrir la carpeta en VS Code
code Pa-Parchar

# 3. Agregar tu API key de OpenAI en finanzas.html línea ~971
